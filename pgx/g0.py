...
# g0 is the original (reference) Go implementation, kept untouched so that the
# optimized `pgx.go` can be regression-tested against it. See pgx/_src/games/g0.py.
...
import pgx._src.games.g0 as g0
...
class State(core.State):
    ...
    def env_id(self) -> core.EnvId:
        return f"g0_{self._size}x{self._size}"  # type: ignore
...
class G0(core.Env):
    ...
    def id(self) -> core.EnvId:
        return f"g0_{int(self._game.size)}x{int(self._game.size)}"  # type: ignore
